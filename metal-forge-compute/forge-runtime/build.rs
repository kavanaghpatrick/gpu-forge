...
fn main() {
    ...
    let metal_file = "shaders/gather.metal";
    let air_file = out_dir.join("gather.air");
    ...
    let metallib_path = out_dir.join("gather.metallib");
    ...
    println!(
        "cargo:rustc-env=GATHER_METALLIB_PATH={}",
        metallib_path.display()
    );
    println!(
        "cargo:warning=Built gather.metallib at {}",
        metallib_path.display()
    );
}
