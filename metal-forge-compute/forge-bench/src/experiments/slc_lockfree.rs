//! SLC-Resident Lock-Free Data Structures.
//!
//! Two GPU-native concurrent data structures running entirely on-chip:
//!
//! Part A: Lock-Free Hash Table
//!   Open-addressing, linear probing (stride-1 per KB finding 3240),
//!   atomic CAS insert, relaxed ordering. EMPTY sentinel = 0xFFFFFFFF.
//!   Capacity = 2x operations for 50% load factor, power-of-2 for bitmask.
//!   At 1M ops the table is 16MB — SLC-resident (KB finding 3231).
//!
//! Part B: MPMC Queue
//!   Lock-free ring buffer with atomic head/tail.
//!   Each thread does 10 enqueue+dequeue cycles.
//!   Per-SIMD atomic aggregation (KB finding 3239).
//!
//! CPU baseline: Rust HashMap (insert/lookup) and VecDeque (queue).
...
pub struct SlcLockfreeExperiment {
    // Hash table GPU buffers
...
impl Experiment for SlcLockfreeExperiment {
    fn name(&self) -> &str {
        "slc_lockfree"
    }

    fn description(&self) -> &str {
        "SLC lock-free: GPU hash table (insert/lookup/mixed) + MPMC queue"
    }
    ...
    fn setup(&mut self, ctx: &MetalContext, size: usize, gen: &mut DataGenerator) {
        ...
        // Generate random keys (avoid EMPTY sentinel)
        self.input_keys = gen
            .uniform_u32(size)
            .into_iter()
        ...
        // Pre-create PSOs
        self.pso_cache
            .get_or_create(ctx.library(), "gpuos_ht_insert");
        self.pso_cache
            .get_or_create(ctx.library(), "gpuos_ht_lookup");
        self.pso_cache
            .get_or_create(ctx.library(), "gpuos_ht_mixed");
        self.pso_cache
    ...
    fn run_gpu(&mut self, ctx: &MetalContext) -> f64 {
        ...
        // Return insert time as primary metric
        self.insert_ms
    }
    ...
    fn validate(&self) -> Result<(), String> {
        // At least 50% of lookups should hit (we inserted all keys)
    ...
    fn metrics(&self, _elapsed_ms: f64, _size: usize) -> HashMap<String, f64> {
        ...
        if self.insert_ms > 0.0 {
            m.insert(
                "insert_mops".to_string(),
                size / self.insert_ms / 1000.0,
            );
        }
        if self.lookup_ms > 0.0 {
            m.insert(
                "lookup_mops".to_string(),
                size / self.lookup_ms / 1000.0,
            );
        }
        if self.mixed_ms > 0.0 {
            m.insert(
                "mixed_mops".to_string(),
                size / self.mixed_ms / 1000.0,
            );
        }

        // Queue: total ops = size * QUEUE_CYCLES * 2 (enqueue + dequeue)
        let total_q_ops = size * QUEUE_CYCLES as f64 * 2.0;
        if self.queue_ms > 0.0 {
            m.insert(
                "queue_mops".to_string(),
                total_q_ops / self.queue_ms / 1000.0,
            );
        }
