...
fn main() {
    ...
    let metal_file = "shaders/filter.metal";
    let air_file = out_dir.join("filter.air");
    ...
    let metallib_path = out_dir.join("filter.metallib");
    ...
    println!(
        "cargo:rustc-env=FILTER_METALLIB_PATH={}",
        metallib_path.display()
    );
    println!(
        "cargo:warning=Built filter.metallib at {}",
        metallib_path.display()
    );
}
