...
- [x] 4.1 Full quality check (clippy, test, build) — all pass clean, no fixes needed

- [x] 4.2 Create PR and verify CI — pushed to existing PR #10
...
- GCIX v3 tests: ChunkMetadata lacks PartialEq, so field-by-field comparison helper needed
- v2 backward compat test: patch v3 file by setting version=2, zeroing bytes[76..92], recomputing CRC over [0..76)
- Empty store produces chunks_bytes=0 in v3 header, load_gcix returns None for chunk_metadata (correct behavior)
- Multi-chunk files (>CHUNK_SIZE) round-trip correctly with per-chunk flags (is_first/is_last) preserved
