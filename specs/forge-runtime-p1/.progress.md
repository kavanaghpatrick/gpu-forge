...
- SortBuffer and FilterBuffer copy_from_slice both require &mut self — all buffer variables need `let mut`
- Standalone ::new() and ::with_context() produce bit-identical sort and filter results — deterministic GPU execution confirmed
- Clippy type_complexity on FilterResult::take_values_buffer — fix with #[allow(clippy::type_complexity)]
- Clippy borrow_deref_ref on PsoCache Bool constant — &*b simplified to b
- Clippy manual_div_ceil in gather.rs — use .div_ceil() method
- forge-bench had 34 pre-existing clippy warnings + 17 doc warnings — all fixed (unused imports, dead code, needless_late_init, unescaped brackets in docs)
