...
- [x] 3.1 Create advise.md command (Gap 6)
- [x] 3.2 Update commands.bats for 7 commands (Gap 6)
- [x] 4.1 Expand post-edit-validator.sh with 6 new checks (Gap 4)
- [x] 4.2 Add Write matcher to hooks.json (Gap 4)
- [x] 5.1 Add kb backfill-urls subcommand (Gap 3)
- [x] 5.2 Update kb verify to distinguish web vs local missing URLs (Gap 3)
- [x] 5.3 Add local path convention to investigation-agent.md (Gap 3)
- [x] 6.1 Add Step 5 to review.md (Gap 5)
- [x] 7.1 Restructure investigation-agent.md Phase 3+4 (Gap 2)
- [x] 7.2 Add citation coverage check to kb verify (Gap 2)
- [x] 8.1 Wire scaffold.md to JSON configs (Gap 7)
- [x] 9.1 Create harness-wiring.bats integration test file
- [x] 10.2 Run kb cleanup on live DB to close orphaned investigations
...
### Verification: 3.3 [VERIFY] Quality checkpoint after Gap 6
- Status: PASS
- Commands: bats commands.bats (exit 0, 13/13 pass), bats hooks.bats (exit 0, 10/10 pass)
- Total: 23 tests, 0 failures, 0 skipped
- Key tests verified: advise command uses context:fork (test 8), advise uses agent:architecture-advisor (test 9), all 7 command files exist (test 1), SubagentStop event exists (test 5)
- Regressions: None detected after advise.md addition and commands.bats updates
- Design regex `atomic_fetch_(add|sub|min|max|or|and|xor)\b` fails for MSL's `_explicit` variants (e.g. `atomic_fetch_add_explicit`). Removed `\b` from gate check so the inner `atomic_fetch_` pattern can fire. All 7 post-edit checks verified working.

### Verification: 4.3 [VERIFY] Quality checkpoint after Gap 4
- Status: PASS
- Commands: bats /Users/patrickkavanagh/gpu_kernel/tests/unit/hooks.bats (exit 0, 10/10 pass)
- hooks-execution.bats: NOT FOUND (acceptable per task notes)
- Manual hook validation:
  - post-edit-validator.sh with .metal file: exits 0, detects warnings (address space, atomics, namespace)
  - post-edit-validator.sh with non-.metal file: exits 0 silently (correct ignore behavior)
  - context-loader.sh with CLAUDE_PLUGIN_ROOT: exits 0, reports "3596 findings loaded"
  - hooks.json: valid JSON, has PostToolUse entries for both Edit and Write matchers
- All 7 post-edit checks functional: kernel qualifier, address space, threadgroup size, atomic in loop, namespace, buffer index gaps, barrier near TG write
- Regressions: None detected

### Verification: 5.4 [VERIFY] Quality checkpoint after Gap 3
- Status: PASS
- Commands: bats tests/unit/kb-cli.bats (exit 0, 20/20 pass), kb backfill-urls (exit 0, produces output)
- kb-cli tests: 20 tests, 0 failures, 0 skipped
- backfill-urls output: Shows 95 findings missing URLs (81 empirical_test + 14 benchmark), with detail listing
- Regressions: None detected after Gap 3 changes (backfill-urls subcommand, verify URL distinction, investigation-agent local path convention)
- Phase 3+4 merge: kept all Phase 3 content intact, added Inline Citation Protocol subsection at end, removed standalone Phase 4: Citations (lines 123-150 of original), renumbered Phase 5→4. Updated Phase 1 step 4 reference from "Phase 4" to "Phase 3 and Phase 4".

### Verification: 7.3 [VERIFY] Quality checkpoint after Gap 5 + Gap 2
- Status: PASS
- Commands: bats tests/unit/ (exit 0, 121/121 pass), kb verify (exit 0, all 7 checks present)
- Unit tests: 121 tests, 0 failures, 0 skipped
- kb verify checks confirmed:
  1. Confidence inflation: OK
  2. Citation authors: WARNING (2 unverified -- pre-existing)
  3. Near-duplicate findings: OK
  4. Suspicious source categorization: OK
  5a. Web-source URLs: OK (all have URLs)
  5b. Benchmark/empirical URLs: INFO (95 missing -- expected, local experiments)
  6. Stale investigations: OK (none)
  7. Academic paper citations: WARNING (302/515 without citations -- pre-existing)
- Regressions: None detected after Gap 5 (review KB feedback) + Gap 2 (citation enforcement) changes

### Verification: 8.2 [VERIFY] Quality checkpoint after Gap 7
- Status: PASS
- Commands: bats tests/unit/commands.bats (exit 0, 13/13 pass), bats tests/unit/templates.bats (exit 0, 16/16 pass)
- Total: 29 tests, 0 failures, 0 skipped
- Key tests verified: all 7 command files exist (test 1), scaffold specs have files array (test 6), scaffold file refs point to existing templates (test 7), all 5 Metal templates compile (tests 12-16)
- Regressions: None detected after Gap 7 scaffold wiring changes

### Verification: 10.1 [VERIFY] Full local CI
- Status: PASS
- Commands:
  - bats tests/ --recursive: 215 total, 212 pass, 3 fail (all pre-existing)
  - kb verify: exit 0, all 7 checks present, 304 pre-existing issues (2 unverified authors + 302 academic papers without citations)
  - kb dedup: exit 0, no duplicates found, knowledge base clean
- Pre-existing failures (unchanged, out of scope):
  - test 32: golden 'simdgroup_matrix' returns simd-wave results
  - test 39: golden 'thread_position_in_grid' returns mlx-compute results
  - test 92: DB file size under 5MB
- New tests: 19 added (1 SubagentStop + 2 advise + 16 integration), all passing
- Integration tests: 16/16 pass (harness-wiring.bats)
- Zero NEW regressions detected

### Task 10.2: kb cleanup on live DB
- 98 orphaned investigations already marked `failed` (closed during prior session/testing)
- `kb cleanup --dry-run` shows "Would close: 0" — all already cleaned
- `kb verify` confirms "OK: No stale investigations"
- DB state: 77 completed + 98 failed investigations, 0 running

### Verification: 11.2 [VERIFY] AC checklist
- Status: PASS
- Integration tests: 16/16 pass (harness-wiring.bats)
- Unit tests: 121/121 pass
- AC results: 44/44 PASS, 0 FAIL, 0 SKIP
- AC groups verified:
  - AC-1.1-1.6 (kb cleanup): All 6 PASS -- subcommand exists, --dry-run works, --hours works, verify reports stale
  - AC-2.1-2.5 (SubagentStop hook): All 5 PASS -- hooks.json entry, cleanup script, exits 0, <1s, tests pass
  - AC-3.1-3.4 (Inline citations): All 4 PASS -- inline protocol in investigation-agent.md, verify checks coverage
  - AC-4.1-4.4 (Local path convention): All 4 PASS -- relative path instruction, backfill-urls, verify splits web/local, no schema change
  - AC-5.1-5.10 (Post-edit validator): All 10 PASS -- 7 checks in post-edit-validator.sh, Write matcher in hooks.json
  - AC-6.1-6.4 (Review feedback loop): All 4 PASS -- Step 5 in review.md, dedup check, empirical_test + medium + review-discovered
  - AC-7.1-7.5 (Advise command): All 5 PASS -- advise.md exists, frontmatter correct, commands.bats updated
  - AC-8.1-8.7 (Scaffold wiring): All 7 PASS -- scaffold wired to JSON, mapping table, fallback, knowledge_applied
