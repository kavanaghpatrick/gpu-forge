...
- [x] 2.2 Add hash table pre-clearing optimization - 645331f (gpu-search-ui)
...
- SearchProfile extended with dedup_us, dedup_table_size, dedup_collisions fields — print() shows dedup line
- dedup() now returns u64 (elapsed microseconds) — dedup_with_error() delegates timing to dedup() directly
- dedup_collisions stays 0 until GPU atomic counter is added (requires new buffer); dedup_table_size = DEDUP_TABLE_CAPACITY constant
- clear_hash_table_kernel added to PATH_SEARCH_SHADER — writes EMPTY_KEY to hash table + zeros flags in parallel
- CPU write_bytes calls removed from new() and dedup() — GPU clear kernel handles it
- Clear kernel dispatched in same command buffer as dedup kernel (no extra commit/wait round-trip)
- Dispatch size: max(DEDUP_TABLE_CAPACITY, capped_match_count) threads — covers both table and flags
- GPU clear adds negligible time to dedup command buffer — single round-trip for both clear + dedup
- test_gpu_clear_hash_table_correctness verifies all 131072 slots set to 0xFFFFFFFF and flags zeroed
- clear_us field added to SearchProfile for profiling the GPU clear kernel contribution

- test_dedup_no_duplicates: loads 150 paths (50 unique x3 repeats), searches "project", verifies 0 duplicate file_paths in results
- GPU dedup correctly marks all duplicate path strings with unique_flags==0; CPU extraction loop skips them
- All 18 engine::search unit tests pass including the new dedup correctness test

- test_dedup.rs uses helper functions (generate_paths_with_duplicates, search_with_dedup) to reduce boilerplate
- 10K scale test: 10K unique + 5K dupes, verifies zero duplicates in results via HashSet
- 50K peak load test: 50K unique + 25K dupes, #[ignore] tag, passes in <300ms
- Latency scaling test: times search at 1K/5K/10K/25K scales with 3-run median, prints table
- Used #[ignore] test instead of Criterion bench — simpler, no new binary target needed
- Edge case tests cover: empty pattern, no chunks loaded, zero-match query, all-duplicates (1000 copies → 1 result), few-unique-many-dupes (5 unique x200 → 5 results)
- All 8 tests pass (6 regular + 2 ignored)
