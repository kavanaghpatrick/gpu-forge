...
- `metal` crate v0.31 re-exports `objc` via `pub extern crate objc` — `metal::objc::msg_send!` is available with no extra Cargo dependency for `-Os` compilation
- `set_bytes()` confirmed in metal 0.31 — signature is `set_bytes(index, length, bytes)` where index is the buffer binding index
- `setOptimizationLevel` NOT exposed in metal 0.31 CompileOptions — must use raw `msg_send![setOptimizationLevel: 1u64]` via metal::objc
- `metal::objc::msg_send!` expands to use `sel!` macro internally — must `use metal::objc::{sel, sel_impl};` to bring those macros into scope for the expansion to work
...
### Verification: 2.3 [VERIFY] Quality checkpoint: build + tests after -Os
- Status: PASS
- Commands: cargo build --release (exit 0), cargo test (exit 0)
- Build: Release build succeeded with 1 benign warning (cfg condition in objc sel_impl macro expansion — not from project code)
- Test results: 96 passed, 0 failed, 7 ignored across 10 test suites
  - lib.rs unit tests (x2 targets): 29 passed each (bridge, index, search, shader)
  - test_bridge.rs: 6 passed (channel roundtrips, backpressure, shutdown)
  - test_engine.rs: 9 passed, 1 ignored (throughput_10mb)
  - test_gpu_correctness.rs: 6 passed, 2 ignored (perf tests)
  - test_index.rs: 5 passed
  - test_integration.rs: 2 passed
  - test_path_search.rs: 10 passed (basic, raw GPU, single path, chunk integrity, 1K, 200K, 1M scale, multi-word, real filesystem, absolute prefix)
  - test_profile.rs: 0 passed, 4 ignored
- Comparison with V1.6: Same test count (96 pass, 7 ignored), zero regressions from -Os compilation change
- No fixes needed

## Next
Task 3.2 Add unit tests for GpuPathMatchResult and score_file_extension_cpu()
