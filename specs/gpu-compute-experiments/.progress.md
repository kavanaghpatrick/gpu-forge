...
- DuckDB CLI approach: write CSV to temp file, shell out with `duckdb -csv -noheader` and pipe SQL via stdin
- DuckDB@1M: GPU=154ms, DuckDB=70ms, ratio=0.5x (SLOWER) -- DuckDB is highly optimized for analytical queries
- DuckDB startup overhead is ~50ms but still faster than GPU pipeline at 1M due to efficient columnar execution
- Graceful skip: check `duckdb --version` in constructor, fall back to CPU HashMap if not installed
- CSV format: no header, two columns (key as INTEGER, value as DOUBLE) -- DuckDB auto-detects with columns= parameter
- read_csv with explicit column types avoids DuckDB type inference overhead
- GPU run_gpu is identical to pipeline experiment (copy-paste of full dispatch chain)
- Validation uses same 1% relative error tolerance as pipeline experiment

## Learnings (task 6.1)

- types.rs already had layout tests for all param structs except CsvBenchParams -- added that plus a comprehensive test_all_params_are_16_bytes
- buffer_pool.rs already had page_align tests -- added specific values from spec, large values, and a full alloc/recycle integration test using real Metal device
- hardware.rs already had bandwidth lookup + utilization tests -- added detect_chip with real Metal device, zero bandwidth edge case, case insensitivity test
- timing.rs already had test_timer_measures_positive -- added millisecond range test and multiple stops test
- pso_cache.rs had no tests -- added new/default/empty checks and HashMap key equality/inequality tests (no GPU needed)
- MTLCreateSystemDefaultDevice no longer requires unsafe block in newer objc2-metal versions
- 31 total unit tests pass for forge-primitives

## Learnings (task 6.2)

- stats.rs, config.rs, data_gen.rs all had existing test modules -- added more tests to each
- IQR outlier removal affects test expectations: values [2,4,4,4,5,5,7,9] had the extreme values removed, so test data must be chosen to avoid unintended outlier removal
- 65 total unit tests pass for forge-bench after additions (was ~40 before)
- Tests added: CV verification (constant/known values), stddev known values, multiple outlier removal, percentile function, parse_sizes (multi), fractional sizes (0.5M), profile ordering, time_series reproducibility/range, csv_records size/newline/reproducibility, zero count edge cases, sequential call differentiation
