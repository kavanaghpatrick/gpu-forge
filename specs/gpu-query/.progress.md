...
- [x] 3.5 Data catalog tree view with column browser
...
- CatalogState: flat-index navigation over tree (tables + expanded columns), resolve_selected() maps flat index to (table_idx, is_column, col_idx)
- CatalogEntry bridges io::catalog::TableEntry to TUI display with ColumnInfo (name + type_name string)
- Format badges: [CSV] green, [PQ] blue, [JSON] yellow; column type colors: INT64 blue, FLOAT64 purple, VARCHAR green, BOOL orange, DATE yellow
- render_catalog_tree takes &mut CatalogState for scroll adjustment; render_frame changed to &mut AppState for this
- 329 lib tests total (312 + 17 catalog)
- CPU comparison estimate: (bytes_scanned / 6.5 GB/s) / gpu_time = speedup. 6.5 GB/s is realistic single-core sequential scan throughput on M4 (DRAM bandwidth is ~60 GB/s but cache hierarchy limits sequential scan)
- CpuEstimate struct + format_row_count (SI suffixes: K/M/B), format_data_bytes (decimal GB), format_speedup (~Nx vs CPU) added to metrics.rs as public API
- build_metrics_performance_line produces full format: "142M rows | 8.4 GB | 2.3ms (cold) | GPU 94% | ~312x vs CPU"
- QueryMetrics gained is_warm field for warm/cold timing display; GpuTimer.into_metrics_with_warmth() for setting it
- AppState.last_query_metrics: Option<QueryMetrics> added for metrics flow from executor to TUI results
- 353 lib tests total (329 + 18 metrics + 6 results)
