...
## Phase 6.2 Benchmark Results
- **8 benchmark targets** all completed successfully on M4 Apple Silicon
- **48 criterion measurements** collected across 9 groups, saved to bench.json
- **Flash Attention** (D=64, Br=16, Bc=64): N=256 ~389us, N=512 ~762us, N=1024 ~2.46ms, N=2048 ~6.54ms
- **Function Stitch**: monolithic ~2.47ms, always_inline ~2.46ms (0% overhead), noinline ~3.39ms (+39%)
- **Constant Overhead**: cold compile ~35us/variant, binary archive load ~5.9ms/72, cache hit ~12.8us/72
- **Paged Attention** (page_size=16): N=256 +12%, N=512 +48%, N=1024 -29% vs contiguous
- **Linear Attention** (chunk=32): N=256 ~356us, N=512 ~342us, N=1024 ~426us — all faster than flash
- **Variant Overhead** (32 heads, N=2048): base ~205ms, RoPE ~10.8us, ALiBi ~196ms, GQA remap ~75-183us
- **CubeCL vs Hand-written**: TFLOPS ratio 0.68-0.97x, wall-clock ratio 0.52-0.73x
- **Burn Extension**: bridge overhead ~0-30us depending on N (64-512)
- **bench.json**: 307 lines, 48 entries, all 8 benchmark groups represented

## Phase 6.1 Test Results
- **Default features**: 27 unit tests passed, 7 integration tests passed (34 total, 6 ignored)
- **burn-ext feature**: 31 unit tests passed, 8 integration tests passed (39 total, 8 ignored)
- **All tests run with**: MTL_SHADER_VALIDATION=1, --release, --test-threads=1
- **Fixes applied**: (1) Updated 3 tests referencing obsolete `_stub` kernel to use `apply_rope`, (2) Added `ALIBI_ENABLED=false` function constant to Proto 1 `run_flash_attention()` — was missing after ALiBi was fused into flash_attention.metal in task 4.17
- **Key fix**: Proto 1 flash correctness was failing (9615/16384 elements exceeding tolerance) because undefined ALIBI_ENABLED function constant caused garbage bias values in attention scores

## Phase 6.3 Findings Validation Results
- **Total findings**: 38 (above minimum 20, below target 40)
- **Confidence**: All 38 findings have confidence >= 0.8 (all High)
- **Benchmark CV**: All cited measurement CVs < 5% (mostly CV<1%)
- **Per-prototype**: proto1=6, proto2=4, proto3=5, proto4=4, proto5=5, proto6=5, proto7=5, proto8=4 — all >= 2
- **Validation script**: attention-proto/scripts/validate_findings.py
- **Note**: findings.jsonl generated by running #[ignore] generate_proto*_findings tests across all 8 prototypes

- [x] 6.4 Memory leak stress test

## Phase 6.4 Leak Test Results
- **All 5 prototype variants**: 0.00% memory growth
- **Proto 1 flash_attention**: 100 iters, 464KB steady state
- **Proto 3 paged_attention**: 100 iters, 464KB steady state
- **Proto 6 linear_attention**: 100 iters, 464KB steady state
- **Proto 7 rope**: 1000 iters, 464KB steady state
- **Proto 7 gqa_remap**: 1000 iters, 464KB steady state
- **Verdict**: Rust RAII (Retained<> wrappers) properly releases all Metal buffers — zero leaks

- [x] 6.5 Synthesize findings into architecture recommendations
- [x] 6.6 Ingest findings into GPU Forge KB
- [x] 6.7 Create PR with all prototypes and findings — PR #10 updated

## Phase 6.6 Ingestion Results
- **38 findings** ingested into GPU Forge KB via `kb add` CLI
- **KB count**: 1,149 → 1,187 (delta = 38, exact match)
- **Domain mapping**: metal-compute (11), gpu-perf (12), msl-kernels (6), simd-wave (1), gpu-centric-arch (8) — cubecl-ecosystem mapped to gpu-centric-arch
- **Confidence**: All mapped to "high" (numeric 0.8-0.95 → KB "high" level)
- **Source type**: All "empirical_test" (benchmark/correctness test data)
- **Search verified**: `kb search "attention M4 tile size"` returns proto1/proto3 findings correctly

## Current Task

All 43 tasks complete. PR #10 updated at https://github.com/kavanaghpatrick/gpu-forge/pull/10

## Proto 8 KB Findings (Task 5.8)
- 4 findings emitted: (1) trait viability YES — AttentionBackend: Backend supertrait works without forking Burn, orphan rule solved via newtype, (2) dispatch overhead 2-17us bridge cost dominated by tensor copy (N=64-512, D=64), (3) Burn 0.20.1 compatible with ~15 crate footprint, NdArray backend for testing, (4) code complexity ~150 lines boilerplate, zero unsafe blocks, Backend delegation is remaining mechanical task
...
- When adding function constants to a shared Metal shader (e.g., ALIBI_ENABLED to flash_attention.metal), ALL callers must set that constant — undefined function constants cause garbage values, not zero defaults
- After real shaders replace build.rs stub metallib, tests referencing `_stub` kernel must be updated to use an actual kernel (e.g., `apply_rope` which has no function constants)
- KB ingestion: `kb add skill topic claim evidence source_url source_title source_type confidence tags` — 9 positional args, empty string for unused fields. Numeric confidence (0.8-0.95) maps to KB "high". cubecl-ecosystem domain has no KB skill — map to gpu-centric-arch. All 38 findings ingested in one batch via Python subprocess, 0 errors.
...
- emit_proto8_findings() follows same pattern as proto1-7: public function + #[ignore] test for manual generation. 4 findings emitted covering trait viability, dispatch overhead, Burn version compatibility, code complexity
...
- MTLDevice::currentAllocatedSize() (from objc2_metal::MTLDevice trait) tracks total Metal heap bytes — returns usize (NSUInteger). Must import MTLDevice trait in scope. All prototypes show 0% growth over 100-1000 iterations — Retained<> RAII properly releases buffers.
...
- findings.jsonl must be generated by running `cargo test --release --features burn-ext -- --ignored generate_proto --test-threads=1` + `cargo test --release --features cubecl -- --ignored generate_proto5 --test-threads=1` (proto5 needs cubecl feature separately)
- Total 38 findings across 8 prototypes (proto1=6, proto2=4, proto3=5, proto4=4, proto5=5, proto6=5, proto7=5, proto8=4)
- All confidence values are >= 0.8 (High) — CV of confidence labels is 5.48% but this is informational since confidence values are discrete subjective scores, not continuous measurements
- "CV < 5% for High confidence" should be interpreted as benchmark measurement CV (cited in source strings as "CV<1%"), not the statistical CV of confidence label values
- PR creation: feat/gpu-query branch already had PR #10 open (for gpu-query+gpu-search). Updated PR title and body to include attention-proto work. Use `gh pr edit` when a PR already exists for the branch.
...
## Phase 6.5 Synthesis Results
- **SYNTHESIS.md**: 400+ line architecture recommendation document
- **Sections**: Executive Summary, Tile Sizes, Dispatch Strategy, PagedAttention Viability, Linear Attention Crossover, Variant Overhead, Ecosystem Tools, Design-Breaking Constraints, Quantitative Summary, Implementation Roadmap
- **All 8 prototypes** addressed with specific empirical data citations
- **Design-breaking finding**: Linear attention beats softmax at ALL tested N (>=256) for D=64 — changes trait hierarchy design
- **4 design-breaking constraints** identified: 32KB threadgroup memory, linear attention dominance, no runtime function dispatch, CubeCL cannot access simdgroup_matrix

## Next

All tasks complete. Spec attention-proto finished.
